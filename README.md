...
| 开机先跑几秒内核文字才出 logo（非调试残留） | 显卡驱动(amdgpu/i915)默认不在 initramfs，内核挂载根分区后才加载它，这之前 plymouth 无图形驱动只能显示文字 | **Ubuntu 26.04 用 dracut**(不是 initramfs-tools！)：bootstrap.sh 自动建 `/etc/dracut.conf.d/91-plymouth-mac.conf` 含 `add_dracutmodules+=" plymouth "` + `force_drivers+=" <主显示卡驱动> "`，再 `sudo dracut -f`；验证 `sudo lsinitrd /boot/initrd.img-$(uname -r) \| grep amdgpu.ko` 应有输出、固件数百个 |
| **混合显卡机型(Intel + AMD/NVIDIA)开机全程黑屏直到登录界面** | plymouth 24 默认遍历 `/dev/dri/card0` → 若 card0 是 amdgpu/nvidia 但**没接显示输出**（屏幕由 i915 eDP 驱动）→ `query_device: Could not initialize heads` → plymouth 整个放弃渲染，开机黑屏、关机也黑屏 | bootstrap.sh **自动检测**: 通过 `/sys/class/drm/card*-*/status` 找出 `connected` 的 connector → 回查它所属 card 的驱动 → 若多卡且主显示是 i915，自动在 `plymouthd.conf` 写 `UseSimpledrm=yes`（plymouth 24 新选项，跳过 DRM 协商直接走 simpledrm framebuffer）；同时 dracut `force_drivers` 只填主显示卡驱动（不能加副卡 amdgpu，否则它抢占 `/dev/dri/card0` 让 plymouth 失败）；验证 `plymouthd --debug --no-daemon` 看 `drm driver:` 是否选中 i915 而非 amdgpu |
