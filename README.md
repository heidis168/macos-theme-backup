...
| 开机先跑几秒内核文字才出 logo（非调试残留） | 显卡驱动(amdgpu/i915)默认不在 initramfs，内核挂载根分区后才加载它，这之前 plymouth 无图形驱动只能显示文字 | **Ubuntu 26.04 用 dracut**(不是 initramfs-tools！)：建 `/etc/dracut.conf.d/90-gpu-early.conf` 写 `force_drivers+=" amdgpu "`，再 `sudo dracut -f`；验证 `sudo lsinitrd /boot/initrd.img-$(uname -r) \| grep amdgpu.ko` 应有输出、固件数百个 |
